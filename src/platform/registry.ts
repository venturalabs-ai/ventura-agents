...
export class AgentRegistry {
  ...
  resolve(capability: string): readonly AgentRegistration[] {
    // ⚡ Bolt: Avoid spreading Map iterables into arrays before filtering to prevent unnecessary intermediate O(N) memory allocations
    const activeCapableAgents: AgentRegistration[] = [];
    for (const agent of this.registrations.values()) {
      if (agent.status === "active" && agent.capabilities.includes(capability)) {
        activeCapableAgents.push(agent);
      }
    }
    return activeCapableAgents;
  }
}
