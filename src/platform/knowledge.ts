...
export class KnowledgeGraph {
  private readonly entities = new Map<string, Entity>(); private readonly edges = new Map<string, Edge[]>();
  ...
  connect(edge: Edge): void {
    ...
    const edgesFrom = this.edges.get(edge.from) ?? [];
    edgesFrom.push(edge);
    this.edges.set(edge.from, edgesFrom);
  }
